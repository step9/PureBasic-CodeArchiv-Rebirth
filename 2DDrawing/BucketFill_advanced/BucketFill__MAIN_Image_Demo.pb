...
For y=GetBucket_Y_BF() To GetBucket_YY_BF()-102
  For x=GetBucket_X_BF() To GetBucket_XX_BF()
    If GetBucketArray_Point_BF(x, y) ; Read the array
      point=Point(x, y)
      If point And ColorDistance_BF(point, color)<30 ; Percent color distance
...
result=BF(150, image_ID, texture_ID,                                
...
path$=#Caisse
texture_ID=LoadImage(#PB_Any, path$)
result=FloodFill_BF(-2, image_ID, texture_ID,
                    550, ; x pos
                    180, ; y pos
...
Create_animation_buffers_image_BF(image_ID) ; Init animation buffers - Double buffering - Flicker free animation
...
Repeat
  ...
  If Delay_BF(0, 10) ; Background refresh delay - Timer 0, Time 10 ms - 100 timer available 0 - 99 - you can change
    ...
    ; - Call function #1 -
    result=BF(80, Get_animation_buffer_hidden_ID_image_BF(), texture_ID, ; Sprite mode - write sprite 
              0,                                  
              0,                                
              x,                           
              y,                             
    ...
    CompilerIf #PB_Compiler_Version=>560
      ...
      ; - GIF frame output
      result=Sprite_CSS_sheet_BF(1, Get_animation_buffer_hidden_ID_image_BF(), temporary_frame_ID,                                     
                                 0,            ; Grab transparence color from sprite pos x - For preset set ..x=-1                      
                                 0,            ; Grab transparence color from sprite pos y - For preset set ..y= the color you want - as sample $FF00FF  
      ...
      ; - GIF frame output
      result=Sprite_CSS_sheet_simple_BF(80, Get_animation_buffer_hidden_ID_image_BF(), temporary_frame_ID,                                     
                                        0,            ; Grab transparence color from sprite pos x - For preset set ..x=-1                      
                                        0,            ; Grab transparence color from sprite pos y - For preset set ..y= the color you want - as sample $FF00FF  
      ...
      RotateSprite_simple_BF(1, Get_animation_buffer_hidden_ID_image_BF(), temporary_frame_ID,
                             0,   ; Grab transparence color from sprite pos x - For preset set ..x=-1                        
                             0,   ; Grab transparence color from sprite pos y - For preset set ..y= the color you want - as sample $FF00FF
                             436,
                             -95,
