...
;- Rotating demo - Rotating sprite and image - Double buffering demo
...
; - Call function #1 -
result=BF(-2, canvas_ID, image_ID)
...
StartDrawing(ImageOutput(Get_animation_buffer_background_ID_canvas_BF()))
...
Repeat
  ...
  If Delay_BF(0, 20) ; Timer 0 - 100 timer available 0 - 99 - you can change
    ...
    result=RotateSprite_simple_BF(mode, Get_animation_buffer_hidden_ID_canvas_BF(), texture_ID,                              
    ...
    If StartDrawing(CanvasOutput(Get_animation_buffer_hidden_ID_canvas_BF())) ; 
    ...
    x+1 : xx+2
    If x>GadgetWidth(canvas_ID)+100 : x=-100  : EndIf
    If xx>GadgetWidth(canvas_ID)+100 : xx=-100 : EndIf
    StartDrawing(ImageOutput(Get_animation_buffer_background_ID_canvas_BF())) ; Draw in the background animation buffer
    Box(x, 120, 100 , 100, $FF+x)
    StopDrawing()
    StartDrawing(CanvasOutput(Get_animation_buffer_hidden_ID_canvas_BF())) ; Draw in the hidden animation buffer
