...
Define win_event, canvas_ID, image_ID, image_1_ID, window_ID, texture_ID, texture_ID 
...
image_ID=CreateImage(#PB_Any, canvas_width, canvas_height) ; Create a image for output

canvas_ID=CanvasGadget(#PB_Any, canvas_x, canvas_y, canvas_width, canvas_height) ; This canvas is for final displaying the results
...
result=BF(-2, image_ID, texture_ID) ; Make a Background texture
...
DrawText(500, 5, "Flicker free animations for canvas", -1)
...
texture_clip_yy=0) ; Endposition inside the texture   - Preset 0 (Texture clipping)
  ...
  Protected temporary_image_ID=CreateImage(#PB_Any, ImageWidth(image_ID), ImageHeight(image_ID), 24, mask_color) ; Create a temporary image
EndMacro

Macro common_after_BF
  Protected result=Texture_universal_output_BF(mode, image_ID, temporary_image_ID, texture_ID, temporary_color,
                                                                       texture_x,
                                                                       texture_y,
                                                                       texture_width,
                                                                       texture_height,
...
; ===========================================================================================  
; AddPathEllipse Image - output- Procedure
; ===========================================================================================  
Procedure AddPathEllipse_Image_BF(mode, image_ID, texture_ID,
                                  x.d,              ; Output x
                                  y.d,              ; Output y
                                  radius_x.d,       ; Radius x
                                  radius_y.d,       ; Radius y
...
; ===========================================================================================  
; AddPathCircle Image - output- Procedure
; ===========================================================================================  
Procedure AddPathCircle_Image_BF(mode, image_ID, texture_ID,
                                 x.d,              ; Output x
                                 y.d,              ; Output y
                                 radius.d,         ; Radius
                                 starting_angle.d, ; Starting angle
...
; =========================================================================================== 
Procedure Ellipse_Image_BF(mode, image_ID, texture_ID,
                           x,        ; Output x
                           y,        ; Output y
                           radius_x, ; Radius x
                           radius_y, ; Radius y
...
; =========================================================================================== 
Procedure RoundBox_Image_BF(mode, image_ID, texture_ID,
                            x,       ; Output x
                            y,       ; Output y
                            width,   ; Width
                            height,  ; Height
...
; =========================================================================================== 
Procedure Box_Image_BF(mode, image_ID, texture_ID,
                       x,      ; Output x
                       y,      ; Output y
                       width,  ; Width
                       height, ; Height
...
; =========================================================================================== 
Procedure DrawRotatedText_Image_BF(mode, image_ID, texture_ID, font_ID,
                                   x,     ; Output x
                                   y,     ; Output y
                                   text$, ; Text
                                   arc,   ; Arc
...
result=AddPathEllipse_Image_BF(mode, image_ID, texture_ID,
                               820,             ; Output x
                               115,             ; Output y
                               350,             ; Radius x
                               75,              ; Radius y
...
result=AddPathCircle_Image_BF(mode, image_ID, texture_ID,
                              690,             ; Output x
                              350,             ; Output y
                              320,             ; Radius
                              0,               ; Starting angle
...
result=Ellipse_Image_BF(mode, image_ID, texture_ID,
                        270,             ; Output x
                        370,             ; Output y
                        240,             ; Radius x
                        305,             ; Radius y
...
image_1_ID=CreateImage(#PB_Any, 350, 230, 24, $FF00FF) ; Create a image for rotated output

Define path$=#Cat
Define texture_ID=LoadImage(#PB_Any, path$)
Define mode=1

result=RoundBox_Image_BF(mode, image_1_ID, texture_ID,
                         0,               ; Output x
                         0,               ; Output y
                         350,             ; Width
                         230,             ; Height
                         15,              ; Curve x
                         15,              ; Curve y
                         -135,            ; Startposition texture output x   - Preset 0
                         -250,            ; Startposition texture output y   - Preset 0
                         texture_width,   ; Endposition texture output xx    - Preset 0 = Full available width
                         texture_height,  ; Endposition texture output yy    - Preset 0 = Full available height
                         texture_clip_x,  ; Startposition inside the texture - Preset 0 (Texture clipping)
                         texture_clip_y,  ; Startposition inside the texture - Preset 0 (Texture clipping)
...
; ============== Rotate the secondary output ==================== 
RotateSprite_BF(image_1_ID,
...
SpriteSimple_BF(1, image_ID, image_1_ID, ; Function for simple using any images as sprites 
                                  0,                       ; Grab transparence color from sprite pos x                           
                                  0,                       ; Grab transparence color from sprite pos y      
                                  680,                     ; Output pos x
                                  180,                     ; Output pos y
                                  450,                     ; You can here resize the output x
                                  300,                     ; You can here resize the output y
                                  100)                     ; Set alpha blending from 1 to 256 - BF set 1=full visible - 256=invisible (BF specific)                       ; Endposition inside the texture   - Preset 0 = full texture height
...
result=RoundBox_Image_BF(mode, image_ID, texture_ID,
                         -100,            ; Output x
                         450,             ; Output y
                         320,             ; Width
                         200,             ; Height
...
result=Box_Image_BF(mode, image_ID, texture_ID,
                    20,              ; Output x
                    80,              ; Output y
                    140,             ; Width
                    230,             ; Height
...
result=DrawRotatedText_Image_BF(mode, image_ID, texture_ID, font_ID,
                                465,            ; Output x
                                -25,            ; Output y
                                "Hello",        ; Text
                                0,              ; Arc
...
StartDrawing(CanvasOutput(canvas_ID)) ; Output
