...
Define win_event, canvas_ID, canvas_1_ID, window_ID, image_ID, texture_ID
...
image_ID=CreateImage(#PB_Any, 350, 230, 24, $FF00FF) ; Create a image for rotated output
canvas_1_ID=CanvasGadget(#PB_Any, -350, -230, 350, 230); Create a canvas for rotated output
CopyContent_Image_Canvas_BF(image_ID, canvas_1_ID)

Define path$=#Cat
Define texture_ID=LoadImage(#PB_Any, path$)
Define mode=1

result=RoundBox_Canvas_BF(mode, canvas_1_ID, texture_ID,
                          0,               ; Output x
                          0,               ; Output y
                          350,             ; Width
                          230,             ; Height
...
CopyContent_Canvas_Image_BF(canvas_1_ID, image_ID)

; ============== Rotate the secondary output ==================== 
RotateSprite_BF(image_ID,
...
SpriteSimple_BF(1, canvas_ID, image_ID, ; Function for simple using any images as sprites 
...
Define texture_ID=LoadImage(#PB_Any, path$)
Define mode=80
ResizeImage(texture_ID, ImageWidth(texture_ID)/5, ImageHeight(texture_ID)/5, #PB_Image_Raw)
